...
- Parque: nombre, codigo, encargado, puntosAcceso y visitantes. Tendrá métodos para habilitar, consultar, modificar y cerrar puntos de acceso; registrar, buscar, modificar y eliminar visitantes; y realizar los cálculos solicitados para el reporte.

- PuntoAcceso: propiedades codigo, nombre, ubicacion, capacidadMaxima y estado. Tendrá constructor, getters y métodos para modificar la capacidad máxima y el estado.

- Visitante: codigoEntrada, nombre, edad, atraccionesVisitadas y puntosAcumulados. Tendrá constructor, getters y métodos para modificar su información.

## 2. ¿Qué tipo deben tener las propiedades y métodos de cada clase?
Las propiedades nombre, codigo, encargado, ubicacion, estado y codigoEntrada serán String. La capacidad máxima, edad, atracciones visitadas y puntos acumulados serán int.
#### puntosAcceso será PuntoAcceso[] y visitantes será ArrayList<Visitante>.
...
La propiedad puntosAcceso de la clase Parque se implementará mediante un arreglo básico de tipo PuntoAcceso[]. Este almacenará objetos PuntoAcceso y tendrá un tamaño fijo de 5 posiciones, debido a que el parque puede tener como máximo cinco puntos de acceso. Las posiciones que todavía no tengan un punto habilitado contendrán null.
...
Los atributos serán private para mantener el encapsulamiento. Los constructores y métodos que necesiten ser utilizados desde otras clases serán public. El método principal será:
#### public static void main(String[] args).

Esto permite acceder y modificar la información mediante los métodos correspondientes
...
Para registrar un visitante se recibirá un objeto Visitante. Para buscar o eliminar se utilizará el código de entrada, y para modificar se necesitará el código junto con los nuevos datos del visitante.
...
Para buscar, se recorrerá el ArrayList comparando el código de entrada de cada visitante hasta encontrar una coincidencia.
